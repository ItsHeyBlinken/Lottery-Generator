/**
 * Data Parser Module
 * Parses historical PowerBall drawing data from a text file
 */
...
/**
 * Parse the PowerBall data file
 * @param {string} filePath - Path to the data file
 * @returns {Promise<object>} - Object with drawings array and any errors
 */
async function parseDataFile(filePath) {
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      result.errors.push(`Data file not found: ${filePath}`);
      return result;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (drawing) {
        ...
        // Only log as error if it's not the header
...
/**
 * Synchronous version of parseDataFile for use in renderer
 * @param {string} filePath - Path to the data file
 * @returns {object} - Object with drawings array and any errors
 */
function parseDataFileSync(filePath) {
  const result = {
    drawings: [],
    errors: [],
    totalLines: 0,
